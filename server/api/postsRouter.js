...
// SEARCH ALL
postsRouter.get("/search/", (req, res) => {
  Post.find({
    $text: { $search: req.query.p }
  })
    .limit(50)
    .sort({ timestamp: -1 })
    .exec()
...
postsRouter.get("/user/:id", (req, res) => {
  ...
  Post.find({ "author.userId": id })
    ...
    .then(posts => {
      ...
      res.status(200).json(posts.map(post => post.postRepr()));
      logger.info(
        chalk.blue(`Retrieved blog posts for the user with ID ${id}`)
      );
    })
    .catch(err => {
      logger.error(chalk.red(err));
...
postsRouter.post("/", authCheck, (req, res) => {
  ...
  User.findOne({ auth0_id: req.body.author.userId })
    .exec()
    .then(user => {
      if (!user) {
